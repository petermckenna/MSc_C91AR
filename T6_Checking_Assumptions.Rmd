...
subtitle: "Computer lab 6: Screening Data & Tests of Normality"
...
- Your job is to tidy up (where required)
    - vector labels
    - cell values
- Check the distribution of the data
    - using `describe`
    - by creating and examining a histogram     
- Check for missing values and replace with the mean
- Plot a transformed version of the data    

# The data

- In the course datasets the file is titled: "handw_messy.csv"
- Download this into your data_raw folder
- This will be the file we are working with today

# Correct metadata

- The data comes from a real dataset of people's heights and weights
- height should be labelled `height_in`
- weight should be labelled `weight_lbs`

# The data

- The data contained in each vector will need to be checked for missing values
- If there are missing values, replace these with the mean of that vector

# Visualisation

- You should use histograms to examine the distribution of each vector
    - you can use either **`ggplot2`** or **`tidyplots`** to do this
- `ggpairs` from the **`GGally`** package will help you examine the relationship between the vectors, and to determine if a transformation is required.
- Time permitting, you should visualise the data post-transformation too.
...
- You did some label tidying
- You applied the test statistics for distribution normality
- You examined the relationship between two variables
